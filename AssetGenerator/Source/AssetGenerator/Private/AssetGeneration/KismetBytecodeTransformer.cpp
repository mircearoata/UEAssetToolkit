...
void FKismetBytecodeTransformer::SetUberGraphTransformer(TSharedPtr<FKismetBytecodeTransformer> Transformer) {
    fgcheck(Transformer->IsUberGraphFunction());
...
TSharedPtr<FKismetCompiledStatement> FKismetBytecodeTransformer::ProcessStatement(TSharedPtr<FJsonObject> Statement) {
    ...
    if (InstructionName == TEXT("Let") || InstructionName == TEXT("LetObj") ||
        ...
        if (ExpressionInstName == TEXT("ObjToInterfaceCast") || ExpressionInstName == TEXT("CrossInterfaceCast")) {
            ...
            //Converts FScriptInterface pointer to one interface into another at the same object
                ...
                fgcheckf(0, TEXT("Unknown interface cast instruction %s"), *ExpressionInstName);
            }
            ...
            fgcheck(InterfaceClass && InterfaceClass->IsChildOf<UInterface>());
        ...
        if (ExpressionInstName == TEXT("DynamicCast") || ExpressionInstName == TEXT("MetaCast")) {
            ...
            //MetaCast handles conversion between UClass types (actually it only does class type checking)
                ...
                fgcheckf(0, TEXT("Unsupported cast instruction: %s"), *ExpressionInstName);
            }
        ...
        if (ExpressionInstName == TEXT("PrimitiveCast")) {
            ...
            fgcheck(CastType == TEXT("InterfaceToBool") || CastType == TEXT("ObjectToBool"));
    ...
    if (InstructionName == TEXT("CallMulticastDelegate")) {
        ...
        if (!bIsSelfContext) {
            ...
            fgcheckf(MemberParentClass, TEXT("Couldn't resolve delegate signature parent class %s"), *MemberParentPath);
        ...
        fgcheckf(Function, TEXT("Cannot resolve delegate signature member %s in parent scope %s"), *MemberName, *MemberParentClass->GetPathName());
        fgcheckf(Function->HasAnyFunctionFlags(FUNC_Delegate), TEXT("Delegate signature function %s is not marked as delegate"), *MemberName);
    ...
    fgcheckf(0, TEXT("Cannot transform instruction into compiled statement: %s"), *InstructionName);
    return NULL;
}
...
TSharedPtr<FKismetTerminal> FKismetBytecodeTransformer::ProcessExpression(TSharedPtr<FJsonObject> Expression) {
    ...
    if (IsVariableInstruction(InstructionName)) {
        ...
        if (InstructionName == TEXT("DefaultVariable")) {
            ...
            fgcheckf(0, TEXT("Unhandled variable instruction: %s"), *InstructionName);
        }
...
TSharedPtr<FKismetTerminal> FKismetBytecodeTransformer::ProcessLiteralExpression(TSharedPtr<FJsonObject> Expression, bool bIsDelimited) {
    ...
    if (InstructionName == TEXT("StructConst")) {
        ...
        fgcheckf(ScriptStruct, TEXT("Cannot resolve struct %s"), *StructPathName);
        ...
        for (const TPair<FString, TSharedPtr<FJsonValue>>& Pair : Properties->Values) {
            ...
            fgcheckf(Property, TEXT("Cannot resolve struct property %s in %s"), *PropertyName, *ScriptStruct->GetPathName());
    ...
    if (InstructionName == TEXT("SoftObjectConst")) {
        ...
        fgcheck(ObjectPathLiteral->Type.PinCategory == UEdGraphSchema_K2::PC_String);
    ...
    if (InstructionName == TEXT("ObjectConst")) {
        ...
        fgcheckf(ReferencedObject, TEXT("Failed to load UObject referenced by Kismet bytecode: %s"), *ObjectPath);
    ...
    if (InstructionName == TEXT("ArrayConst") || InstructionName == TEXT("SetConst")) {
        ...
        for (const TSharedPtr<FJsonValue>& ValueExpression : Values) {
            ...
            fgcheck(ValueConstant->bIsLiteral);
    ...
    if (InstructionName == TEXT("MapConst")) {
        ...
        for (const TSharedPtr<FJsonValue>& PairObject : Values) {
            ...
            fgcheck(KeyTerminal->bIsLiteral);
            fgcheck(ValueTerminal->bIsLiteral);
    ...
    fgcheckf(0, TEXT("Found unsupported literal instruction while parsing expression: %s"), *InstructionName);
    return NULL;
}
...
TSharedPtr<FKismetCompiledStatement> FKismetBytecodeTransformer::ProcessFunctionCallStatement(TSharedPtr<FJsonObject> Statement)
{
    ...
    fgcheck(IsCallFunctionInstruction(InstructionName));
    ...
    if (InstructionName != TEXT("CallMath")) {
        ...
        fgcheckf(ContextObjectClass, TEXT("Failed to resolve context class for math function, class: %s"), *ContextClassPath);
    }

    UFunction* ResolvedFunction = ContextObjectClass->FindFunctionByName(*FunctionName);
    fgcheckf(ResolvedFunction, TEXT("Couldn't resolve function %s in context of class %s"), *FunctionName, *ContextObjectClass->GetPathName());
    ...
    if (InstructionName == TEXT("LocalVirtualFunction") || InstructionName == TEXT("VirtualFunction")) {
        ...
        fgcheckf(0, TEXT("Unhandled function call opcode: %s"), *InstructionName);
    }
    ...
    if (FunctionName == ExecuteUbergraphFunctionName) {
        fgcheck(ContextObjectClass == OwnerBlueprint->SkeletonGeneratedClass);
        
        //Calls into ubergraph should never appear inside of the ubergraph itself
        fgcheckf(!IsUberGraphFunction(), TEXT("Encountered a call into ubergraph inside of the ubergraph itself. This should never happen"));
        fgcheck(UberGraphTransformer.IsValid());
    ...
    if (LatentInfoParameterIndex != -1) {
        ...
        fgcheckf(IsUberGraphFunction(), TEXT("Encountered latent action outside of the uber graph function: %s"), *CurrentFunctionName);
        ...
        fgcheck(LatentActionInfo.ExecutionFunction.ToString() == CurrentFunctionName);
...
TSharedPtr<FKismetTerminal> FKismetBytecodeTransformer::ProcessFunctionParameter(TSharedPtr<FJsonObject> ParameterExpression) {
    ...
    if (ParameterInstructionName == TEXT("ArrayGetByRef")) {
        ...
        fgcheck(InnerElementType.ContainerType == EPinContainerType::Array);
    ...
    //because actually it is a perfectly pure node just returning value depending on the inputs
        ...
        for (const TSharedPtr<FJsonValue>& CaseValue : Cases) {
            ...
            fgcheck(CaseTestValue->bIsLiteral);
    ...
    //but it still works, because native static functions don't use context object at all.
        ...
        fgcheck(InlineFunctionCall->FunctionContext.IsValid() == false);
        fgcheck(InlineFunctionCall->FunctionToCall->HasAllFunctionFlags(FUNC_Final | FUNC_Static | FUNC_Native));
    ...
    fgcheck(ParameterTerminal.IsValid());
