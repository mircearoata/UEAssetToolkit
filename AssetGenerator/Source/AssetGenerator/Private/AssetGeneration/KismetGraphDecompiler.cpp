...
UEdGraphNode* FKismetGraphDecompiler::CreateMakeMapNode() {
    ...
    for (int32 i = 0; i < Statement->RHS.Num(); i++) {
        ...
        fgcheck(MakeMapInputPin->Direction == EEdGraphPinDirection::EGPD_Input);
...
UEdGraphNode* FKismetGraphDecompiler::CreateMakeSetNode() {
    ...
    for (int32 i = 0; i < Statement->RHS.Num(); i++) {
        ...
        fgcheck(MakeSetInputPin->Direction == EEdGraphPinDirection::EGPD_Input);
...
UEdGraphNode* FKismetGraphDecompiler::CreateMakeArrayNode() {
    ...
    for (int32 i = 0; i < Statement->RHS.Num(); i++) {
        ...
        fgcheck(MakeArrayInputPin->Direction == EEdGraphPinDirection::EGPD_Input);
...
UEdGraphNode* FKismetGraphDecompiler::CreateCastNode(bool bIsMetaCast) {
    ...
    fgcheck(ObjectToBool->Type == ECompiledStatementType::KCST_ObjectToBool);
...
UEdGraphNode* FKismetGraphDecompiler::CreateAssignmentNode() {
    ...
    if (VariableAssigned->Context && VariableAssigned->Context->ContextType == FKismetTerminal::EContextType_Struct) {
        ...
        fgcheck(Context->Type.PinCategory == UEdGraphSchema_K2::PC_Struct);
        ...
        if (IsLocalVariable(Context)) {
            ...
            if (ContextPropertyName.StartsWith(TEXT("K2Node_MakeStruct"))) {
                ...
                fgcheck(!IsUserCreatedLocalVariable(StructContext));
    ...
    if (VariableAssigned->Context && VariableAssigned->Context->ContextType == FKismetTerminal::EContextType_Object) {
        const TSharedPtr<FKismetTerminal> Context = VariableAssigned->Context;
        fgcheck(Context->Type.PinCategory == UEdGraphSchema_K2::PC_Object);
...
UEdGraphNode* FKismetGraphDecompiler::CreateMakeStructNode(TSharedPtr<FKismetTerminal> StructTerminal) {
    ...
    fgcheck(StructTerminal->ContextType == FKismetTerminal::EContextType_Struct);
    fgcheck(StructTerminal->Type.PinCategory == UEdGraphSchema_K2::PC_Struct);
...
UEdGraphNode* FKismetGraphDecompiler::CreateSetFieldsInStructNode(TSharedPtr<FKismetTerminal> StructTerminal) {
    ...
    if (LastStatement->Type == ECompiledStatementType::KCST_Assignment) {
        ...
        if (IsLocalVariable(LeftSideTerminal) && LeftSideTerminal->AssociatedVarProperty.StartsWith(TEXT("K2Node_SetFieldsInStruct_StructOut"))) {
            ...
            fgcheck(LastStatement->RHS[0]->operator==(*StructTerminal));
            fgcheck(!IsUserCreatedLocalVariable(LeftSideTerminal));
...
UEdGraphNode* FKismetGraphDecompiler::CreateStructMemberSetNode(TSharedPtr<FKismetTerminal> StructTerminal) {
    const FVector2D NodePosition = EditorGraph->GetGoodPlaceForNewNode();
    fgcheck(IsUserCreatedLocalVariable(StructTerminal));
    fgcheck(StructTerminal->ContextType == FKismetTerminal::EContextType_Struct);
    fgcheck(StructTerminal->Type.PinCategory == UEdGraphSchema_K2::PC_Struct);
    ...
    fgcheckf(LocalVariable, TEXT("Local variable in question not found: %s"), *LocalVariableName);
    
    fgcheck(LocalVariable->VarType.PinCategory == UEdGraphSchema_K2::PC_Struct);
    ...
    fgcheck(StructTypeByTerminal == StructTypeByLocalVariable);
...
UEdGraphNode* FKismetGraphDecompiler::CreateCopyNode() {
    ...
    fgcheck(LeftHandOperand.IsValid());
...
UEdGraphNode* FKismetGraphDecompiler::CreateDelegateSetNode() {
    ...
    fgcheckf(0, TEXT("Unsupported K2 Node encountered: UK2Node_DelegateSet"));
    return NULL;
}
...
UEdGraphNode* FKismetGraphDecompiler::CreateVariableSetNode() {
    ...
    fgcheck(ContextTypeStruct->FindPropertyByName(*AccessedPropertyName));
    ...
    if (PossiblySelfPin) {
        ...
        fgcheck(ContextTerminal.IsValid());
    ...
    if (MemberSetK2Node->HasLocalRepNotify()) {
        ...
        fgcheck(RepNotifyStatement->Type == ECompiledStatementType::KCST_CallFunction);
        fgcheck(RepNotifyStatement->FunctionToCall);
        fgcheck(RepNotifyStatement->FunctionToCall->GetFName() == MemberSetK2Node->GetRepNotifyName());

        //Make sure functions have matching context if we are using context at all
        fgcheck(CallContext.IsValid() == AssignedTerminal->Context.IsValid());
        if (AssignedTerminal->Context.IsValid()) {
            fgcheck(CallContext->operator==(*AssignedTerminal->Context));
        }
    }
...
UEdGraphNode* FKismetGraphDecompiler::CreateMakeDelegateNode() {
    ...
    fgcheck(AssignedDelegate.IsValid());
    fgcheck(IsLocalVariable(AssignedDelegate));
    fgcheck(AssignedDelegate->AssociatedVarProperty.StartsWith(TEXT("K2Node_CreateDelegate_OutputDelegate")));
    ...
    fgcheck(FunctionNameTerminal->bIsLiteral);
...
UEdGraphNode* FKismetGraphDecompiler::CreateMulticastDelegateNode(TSubclassOf<UK2Node_BaseMCDelegate> NodeClass) {
    ...
    if (DelegatePin != NULL) {
        fgcheck(DelegateStatement->RHS.Num());
...
UEdGraphNode* FKismetGraphDecompiler::CreateFunctionCallNode() {
    ...
    fgcheck(CallFunctionStatement->Type == ECompiledStatementType::KCST_CallFunction);
...
UStruct* FKismetGraphDecompiler::ResolveContextTerminalType(TSharedPtr<FKismetTerminal> Terminal) {
    ...
    if (Terminal->Type.PinCategory == UEdGraphSchema_K2::PC_Struct) {
        fgcheck(Terminal->ContextType == FKismetTerminal::EContextType_Struct);
    ...
    if (Terminal->Type.PinCategory == UEdGraphSchema_K2::PC_Interface) {
        fgcheck(Terminal->ContextType == FKismetTerminal::EContextType_Object);
        UClass* InterfaceClass = CastChecked<UClass>(Terminal->Type.PinSubCategoryObject);

        fgcheck(InterfaceClass->IsChildOf<UInterface>());
    ...
    if (Terminal->Type.PinCategory == UEdGraphSchema_K2::PC_Class) {
        fgcheck(Terminal->ContextType == FKismetTerminal::EContextType_Class);
    ...
    fgcheckf(0, TEXT("Unknown context terminal type: Category = %s, Sub Category = %s"), *Terminal->Type.PinCategory.ToString(), *Terminal->Type.PinSubCategory.ToString());
    return NULL;
}
